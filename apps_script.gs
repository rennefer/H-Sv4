// === Horus and Seth — submissions collector (Thoth training-data + wenamun_annotator.html,
//     which now covers both the Wenamun and Tale of Two Brothers datasets) ===
...
// just adding Wenamun and/or Tale of Two Brothers support:
// 1. Open the same spreadsheet > Extensions > Apps Script.
// 2. Replace the existing code with this whole file (this keeps everything
//    training-data.html already relies on — it only adds new 'Wenamun' and
//    'TwoBrothers' sheets, written to whenever that dataset is picked in the
//    annotator's dataset dropdown).
...
function doPost(e) {
  ...
  if (project === 'twobrothers') {
    return handleTwoBrothers(data);
  }
...
// New: Tale of Two Brothers manual word annotation — same shape as Wenamun,
// written to its own 'TwoBrothers' sheet.
function handleTwoBrothers(data) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName('TwoBrothers');
  if (!sheet) {
    sheet = ss.insertSheet('TwoBrothers');
    sheet.appendRow(['Timestamp', 'Session ID', 'Annotator', 'Page', 'Line', 'Label', 'Word Count', 'Words Summary', 'Raw JSON']);
  }
